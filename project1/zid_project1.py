...
def get_tics(pth):
    ...
    """
    with open(pth, 'r') as newdoc:
        dic = {}
        for line in newdoc:
            ex, tic = list(line.lower().replace('\"', '').split('='))
            ex, tic = ex, tic.strip()
            dic[tic] = ex
        return dic
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        dic[column] = line[i: i + COLWIDTHS[column]]
        i += COLWIDTHS[column]
    return dic
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ...
        for line in lines:
            dic_to_save = {}
            for col in col_lst:
                dic_to_save[col] = line_to_dict(line)[col]
            data.append(dic_to_save)
...
def create_json(data_dict, pth):
    ...
    """
    with open(pth, 'w') as newdoc:
        json.dump(data_dict, newdoc)
